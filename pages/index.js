...
export default function Home() {
  ...
  const strength = isLowercase + isUppercase + isNumber + isSpecial;
   ...
   /* generates password given a length, running a loop for each character slot */
  useEffect(() => {
    if (submit) {
      if (!params == "") {
  ...
  function Header() {
    return <h1 className="text-center text-5xl p-10">Password Generator</h1>;
  }
    ...
    return (
      <>
        <div className="flex flex-row text-2xl pb-4 items-center">
          Character Length
          <p onChange={handleLengthChange} className="ml-[auto] mr-0 text-4xl">
            {input}
          </p>
        </div>
        <input
  ...
  /* consider abstracting parameters and submitbutton, as well as pwfield and both file saves*/
